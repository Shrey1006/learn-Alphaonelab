...
async def api_register(req, env):
    ...
    return ok(
        {"token": token, "user": {"id": uid, "username": username, "name": name, "role": role}},
...
async def api_login(req, env):
    ...
    stored_username = await decrypt_aes(row.username, enc)
    ...
    real_role = await decrypt_aes(row.role, enc)
    ...
    return ok(
        {"token": token, "user": {"id": row.id, "username": stored_username, "name": real_name, "role": real_role}},
        "Login successful",
    )
