...
from security_utils import decrypt_aes, encrypt_aes, new_id, verify_token
...
async def api_list_activities(req, env):
    ...
    if tag:
        ...
        res = await env.DB.prepare(
            base_q + " JOIN activity_tags at2 ON at2.activity_id=a.id WHERE at2.tag_id=? ORDER BY a.created_at DESC"
        ).bind(tag_row.id).all()
    elif atype and fmt:
        res = await env.DB.prepare(base_q + " WHERE a.type=? AND a.format=? ORDER BY a.created_at DESC").bind(atype, fmt).all()
    elif atype:
        res = await env.DB.prepare(base_q + " WHERE a.type=? ORDER BY a.created_at DESC").bind(atype).all()
    elif fmt:
        res = await env.DB.prepare(base_q + " WHERE a.format=? ORDER BY a.created_at DESC").bind(fmt).all()
    else:
...
async def api_get_activity(act_id: str, req, env):
    ...
    if user:
        enrollment = await env.DB.prepare(
            "SELECT id,role,status FROM enrollments WHERE activity_id=? AND user_id=?"
    ...
    return json_resp(
        {
            ...
            "is_host": is_host,
            "enrollment": {"role": enrollment.role, "status": enrollment.status} if enrollment else None,
        }
    )
...
async def api_create_session(req, env):
    ...
    owned = await env.DB.prepare("SELECT id FROM activities WHERE id=? AND host_id=?").bind(act_id, user["id"]).first()
    if not owned:
...
async def api_list_tags(_req, env):
    ...
    return json_resp({"tags": [{"id": r.id, "name": r.name} for r in (res.results or [])]})
